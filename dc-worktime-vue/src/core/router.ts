...
export const reconstructionRouter = (routeRaws: any[]) => {
	...
	return routeRaws.map((item) => {
		// 在 layout.modules 数组中查找路径与 item.routerUrl 相匹配的模块
		const meta = layout.modules.find((it) => it.path == "/" + item.routerUrl);
	...
	for (const item of menus) {
		...
		if (item.children && item.children.length > 0) {
			...
			const routerRaw = {
				...
				component: () => import("@/composables/empty.vue"), // 路由组件，使用动态导入
				redirect: item.fullPath + "/" + item.children[0].routerUrl, // 重定向路径
				children: [], // 子路由数组
				meta: {
					title: item.title, // 路由标题
					icon: item.icon, // 路由图标
					isShow: item.isShow // 是否显示路由
				}
			};
			...
			router.addRoute(pName, {
				...
				meta: {
					title: item.title, // 路由标题
					icon: item.icon, // 路由图标
					isShow: item.isShow == true // 是否显示路由
				}
			});
			// 如果当前菜单项需要显示
			if (item.isShow == 1) {
