<template>
	<BasePage v-bind="page" :asideLeftWidth="280">
		...
		<template #pageTitle>产品问题列表</template>
		<template #pageBody>
			<div
				class="flex flex-justify-between flex-items-center m-t-0px m-b-24px p-b-10px"
				style="border-bottom: 1px solid rgba(0, 0, 0, 0.06)"
			>
				<div class="title font-size-18px font-b font-bold">产品问题列表</div>
				<div>
...
<script setup lang="tsx">
	...
	const add = () => {
		createDrawerAsync(
			...
			{ refreshList },
			<Detail projectId={projectName.value} userList={userList.value} />
		);
	};
	const handleEdit = (row: any) => {
		...
		createDrawerAsync(
			...
			{ refreshList },
			<Detail projectId={projectName.value} rowInfo={row} userList={userList.value} />
		);
	};
	...
	page.form.onSearch = async (data) => {
		...
		try {
			...
			const res = await selectProductPage({
				...params,
	...
	onMounted(async () => {
		...
		await getTreeData(); // 等待树数据加载完成
		await nextTick(async () => {
			if (projectName.value) {
				const targetNode = findNodeById(treeData.value, projectName.value);
				if (targetNode && tree.value) {
					await nextTick(() => {
						if (tree.value) {
							tree.value.setCurrentKey(targetNode.id);
						}
						handleNodeClick(targetNode, {});
					});
				} else {
					console.log("未找到目标节点，projectName.value:", projectName.value);
				}
			} else {
				if (treeData.value && treeData.value.length > 0) {
					const firstNode = treeData.value[0];
					if (tree.value) {
						tree.value.setCurrentKey(firstNode.id);
					}
	...
	const getTreeData = async () => {
		const res = await selectListCount({ assigned_to_user_type: 4 });
		treeData.value = res;
		treeData.value = markNodeLevel(treeData.value);
		// await page.form.onSearch(page.form.value);
	};
	...
	const handleNodeClick = async (data: any, node: any) => {
		projectName.value = data.id;
		appInstance.setSelectProjectTestIndex(data.id);
		await page.form.onSearch(page.form.value);
		// await getSeverityCount();
		// 重新获取用户列表
	...
	const renderTreeNode = (h, { node, renderParams, store }) => {
		...
		return (
			<span class="tree-node">
				...
				{data.severity_no > 0 && <span class="task-plan-count">{data.severity_no}</span>}
			</span>
		);
	};
