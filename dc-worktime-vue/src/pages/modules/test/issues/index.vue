<template>
	<BasePage v-bind="page" :asideLeftWidth="280">
		...
		<template #pageTitle>开发问题列表</template>
		<template #pageBody>
			<div
				class="flex flex-justify-between flex-items-center m-t-0px m-b-24px p-b-10px"
				style="border-bottom: 1px solid rgba(0, 0, 0, 0.06)"
			>
				<div class="title font-size-18px font-b font-bold">开发问题列表</div>
				<div>
					<ElButton type="danger" :icon="Delete" @click="batchDelete"> 删除 </ElButton>
					<ElButton type="success" :icon="CirclePlus" @click="add"> 提bug </ElButton>
				</div>
			</div>
			...
			<div class="statistics-box">
				<div class="statistics-item">
					...
					<div class="statistics-item-content">
						...
						<div class="statistics-item-title">轻微BUG数量</div>
					</div>
				</div>
				<div class="statistics-item">
					...
					<div class="statistics-item-content">
						...
						<div class="statistics-item-title">一般BUG数量</div>
					</div>
				</div>
				<div class="statistics-item">
					...
					<div class="statistics-item-content">
						...
						<div class="statistics-item-title">严重BUG数量</div>
					</div>
				</div>
				<div class="statistics-item">
			...
			>
				...
				<ElTableColumn prop="assigned_to_user_type" label="指派至" align="center" width="80">
					<!-- 1：前端 2：后端 3：部署 -->
					<template #default="{ row }">
						<template v-if="row.assigned_to_user_type == 1">前端</template>
						<template v-if="row.assigned_to_user_type == 2">后端</template>
						<!-- <template v-if="row.assigned_to_user_type == 3"
							>部署</template
						> -->
					</template>
				</ElTableColumn>
				...
				<ElTableColumn prop="statusName" label="状态" align="center" width="110">
					<template #default="{ row }">
						...
						<template v-if="row.status == 5 && row.retestsType == 2">这不是bug</template>
				...
				<ElTableColumn prop="actions" label="操作" width="180px" align="center" fixed="right">
					<template #default="{ row }">
						...
						<template v-if="row.status == '5'">
							<ElButton size="small" plain @click.stop="handleActivation(row)">激活BUG</ElButton>
						</template>
...
<script setup lang="tsx">
	...
	import Activation from "./dialog/activation.vue";
	...
	const add = () => {
		createDrawerAsync(
			...
			{ refreshList },
			<Detail moudleList={treeData} projectId={projectName.value} />
		);
	};
	const handleEdit = (row: any) => {
		...
		createDrawerAsync(
			...
			{ refreshList },
			<Detail moudleList={treeData} projectId={projectName.value} rowInfo={row} />
		);
	};
	...
	const handleUnsolved = (row: any) => {
		createModelAsync(
			...
			{ refreshList },
			<Unsolved rowInfo={row} />
		);
	};
	const handleActivation = (row: any) => {
		createModelAsync(
			{ title: "激活BUG", showNext: false, width: "1056px", closeOnClickModal: false, closeOnPressEscape: false },
			{ refreshList },
			<Activation rowInfo={row} />
		);
	};
	const handleAssign = (row: any) => {
		createModelAsync(
			{ title: "指派", showNext: false, width: "1056px", closeOnClickModal: false, closeOnPressEscape: false },
			{ refreshList },
			<Assign rowInfo={row} />
		);
	};
	...
	const handleDetail = (index: any, row: any, showBtn: any) => {
		createDrawerAsync(
			...
			<DetailPage type={index} id={row.id} showBtn={showBtn} />
		);
	};
	...
	const getSeverityCount = async (val) => {
		console.log("111111111111111page.form.value", page.form.value);
		try {
			const res = await severityCountBean({ pro_project_id: projectName.value, ...val });
	...
	onMounted(async () => {
		const res = await selectListCount({});
		options.value = res;
