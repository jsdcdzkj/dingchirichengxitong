    ...
    >
      <template #reference>
        <ElInput
          ...
          <template #suffix>
            <el-icon class="el-input__icon"  v-show="showClear" style="cursor:pointer" @click.stop="clear"><Close /></el-icon>
          </template>
        </ElInput>
      </template>
            ...
            <table class="el-month-table" style="">
              ...
              <tr v-for="line in lineCount" :key="line">
                <td v-for="index in (line * 4 <= viewList.length ? 4 : viewList.length - (line - 1) * 4)" :key="index" :class="{ today: viewList[(line - 1) * 4 + index - 1].current, current: viewList[(line - 1) * 4 + index - 1].active }">
                  <div><a class="cell"  :class="{textDis: isOverTime(viewList[(line - 1) * 4 + index - 1].year, viewList[(line - 1) * 4 + index - 1].quarter)}" @click="clickItem(viewList[(line - 1) * 4 + index - 1])">{{ viewList[(line - 1) * 4 + index - 1].label }}</a></div>
                </td>
              </tr>
              </tbody>
            </table>
...
const formatTo = (data, pattern) => {
  return pattern
    ...
    .replace('qq', `0${data[1]}`)
    .replace('q', `${data[1]}`)
    .replace('Q', '一二三四'.substr(data[1] - 1, 1));
};
...
const findText = (str, pattern, find) => {
  ...
  if (find === 'Q') return '一二三四'.indexOf(val) + 1;
  return parseInt(val);
};
