...
if hasattr(sys, '_MEIPASS'):
    icon_path = os.path.join(sys._MEIPASS, 'jsdc.icns')  # 打包后的临时文件夹路径
else:
    icon_path = 'jsdc.icns'  # 开发环境中的相对路径
# 获取本机 IP
def get_local_ip():
...
class LoginWindow(QWidget):
    def __init__(self):
        ...
        if is_already_running():
            QMessageBox.warning(None, "警告", "程序已在运行中", QMessageBox.Ok)
            sys.exit(1)
...
def set_autostart(enabled=True):
    ...
    plist_filename = f"com.mycompany.{app_name}.plist"
...
<dict>
    <key>Label</key>
    <string>com.mycompany.{app_name}</string> 
...
def is_already_running():
    """检查是否已有实例在运行"""
    # 方法1: 使用QSharedMemory (跨平台)
    shared_mem = QSharedMemory("鼎驰工时系统_unique_key")
