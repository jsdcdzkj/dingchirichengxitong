...
def build():
    ...
    # 创建 .dmg 文件
    create_dmg(dist_folder, app_name)


def create_dmg(dist_folder, app_name):
    dmg_name = f"{app_name}.dmg"
    app_path = os.path.join(dist_folder, f"{app_name}.app")
    temp_dmg_folder = os.path.join(dist_folder, "temp_dmg")

    # 创建临时文件夹
    os.makedirs(temp_dmg_folder, exist_ok=True)

    # 复制 .app 文件到临时文件夹
    shutil.copytree(app_path, os.path.join(temp_dmg_folder, f"{app_name}.app"))
    ...
    # 删除临时文件夹
    shutil.rmtree(temp_dmg_folder)

    print(f"{dmg_name} 创建完成，位于当前目录")
